...
import { getProofSubmission, saveProofSubmission, getTestChecklist } from "@/lib/storage";
import { useToast } from "@/hooks/use-toast";
import { Copy, CheckCircle, Circle, Rocket } from "lucide-react";
...
------------------------------------------`;
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <Badge variant={isShipped ? "default" : "secondary"} className="text-sm gap-1.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {STEP_NAMES.map((name, i) => (
              <div key={i} className="flex items-center gap-2 text-sm">
                {allTestsPassed ? <CheckCircle className="h-4 w-4 text-success" /> : <Circle className="h-4 w-4 text-muted-foreground" />}
                <span>{name}</span>
              </div>
            ))}
          </div>
      ...
      <Card className="card-premium">
        ...
        <CardContent>
          ...
          {!allTestsPassed && <p className="text-sm text-warning mt-2 font-medium">Complete all tests before shipping.</p>}
        </CardContent>
      </Card>
      ...
      <Card className="card-premium">
        ...
        <CardContent className="space-y-4">
          <div>
            <label className="text-sm font-medium mb-1 block text-muted-foreground">Lovable Project Link</label>
          ...
          <div>
            <label className="text-sm font-medium mb-1 block text-muted-foreground">GitHub Repository</label>
          ...
          <div>
            <label className="text-sm font-medium mb-1 block text-muted-foreground">Deployed URL</label>
